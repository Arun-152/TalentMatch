...
class AnalysisSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Analysis
        fields = ['id', 'resume', 'job_description', 'matched_skills', 'missing_skills', 'compatibility_score', 'llm_explanation', 'status', 'created_at']
        read_only_fields = ['id', 'matched_skills', 'missing_skills', 'compatibility_score', 'llm_explanation', 'status', 'created_at']

    def to_representation(self, instance):
        ret = super().to_representation(instance)
        # Add human-readable fields for the frontend History list
        ret['candidate_name'] = instance.resume.candidate_name or (instance.resume.file.name if instance.resume.file else 'Raw Text Upload')
        ret['job_title'] = instance.job_description.title
        
        # Inject recommendations dynamically
